...
class TrainVQGAN:
    def __init__(self, args):
        ...
        self.opt_vq, self.opt_dics = self.configure_optimizers(args)
        ...
        for epoch in range(args.epochs):
            with tqdm(range(len(train_dataset))) as pbar:
                for i, imgs in zip(pbar, train_dataset):
                    ...
                    perceptual_loss = sef.perceptual_loss(imgs, decoded_images)
                    rec_loss = torch.abs(imgs - decode_images)
                    perceptual_rec_loss = args.perceptual_loss_factor*perceptual_loss +args.rec_loss_factor*rec_loss
                    perceptual_rec_loss = perceptual_loss.mean()
                    g_loss = -torch.mean(disc_fake)

                    A = self.vqgan.calculate_lamda(perceptual_rec_loss, g_loss)
                    vq_loss = perceptual_loss + q_loss + disc_factor*A*g_loss
                    ...
                    pbar.set_postfix(
                        ...
                        GAN_loss = np.round(gan_loss().detach().numpy().item(), 3)
                )
...
if __name__ == '__main__':
    ...
    args.dataset_path = r"C:\Users\pedro\OneDrive\Área de Trabalho\vq_gan\flowers\rose"
